...
from recbole.model.layers import FMEmbedding, MLPLayers, ContextSeqEmbLayer, SequenceAttLayer
...
class InterestExtractorNetwork(nn.Module):
    ...
    def auxiliary_loss(self, h_states, click_seq, noclick_seq, keys_length):
        ...
        # click predict
        click_prop = self.auxiliary_net(click_input.view(batch_size * hist_length, -1)).view(-1, 1)
        # click label
        click_target = torch.ones(click_prop.shape, device=click_input.device)

        # non-click predict
        noclick_prop = self.auxiliary_net(noclick_input.view(batch_size * hist_length, -1)).view(-1, 1)
        # non-click label
